...
class DepositController extends Controller
{
    ...
    public function approve(Request $request)
    {
        ...
        if($deposit->exam_id){
            ...
            $transaction->charge = getAmount($deposit->charge);
            $transaction->trx_type = '+';
            $transaction->details = 'Payment of exam fee via ' . $deposit->gateway_currency()->name;
            $transaction->trx =  $deposit->trx;
            $transaction->save();
        ...
        if($deposit->exam_id == null){
            ...
            $exam = Exam::findOrFail($deposit->exam_id);
            notify($user, 'PAYMENT_APPROVE', [
                'title' => $exam->title,
                'type' => $exam->question_type == 1 ? 'MCQ':'Written',
                'mark' => $exam->totalmark,
                'link' => url('participate/exam/'.$exam->id),
                'method_name' => $deposit->gateway_currency()->name,
                'method_currency' => $deposit->method_currency,
                'method_amount' => getAmount($deposit->final_amo),
                'amount' => getAmount($deposit->amount),
